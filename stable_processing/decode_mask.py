...
def logits_to_mask(logits:torch.Tensor, threshold = 0.5) -> torch.Tensor:
    ...
    binary_tensor = torch.where(filtered_logits < threshold, torch.tensor(0.0), torch.tensor(1.0))
...
def alpha_mask_generation(original_image_shape:np.ndarray, masks:torch.Tensor) -> torch.Tensor:
    ...
    #print('image_large shape before:', image_large.shape)
    # image_large = image_large.squeeze()
    image_large = image_large.squeeze()
    #print('image_large shape is:', image_large.shape)
    
    if image_large.dim() == 2:
        image_large = image_large.unsqueeze(0)  # 添加一个维度，使其变成3D
...
def hard_mask(alpha_masks: np.ndarray,   original_image: Image.Image) -> Image.Image:
    ...
    #  # 计算未被覆盖的区域掩码
    # combined_mask = np.sum(alpha_masks, axis=0) > 0
    # inverse_mask = np.logical_not(combined_mask).astype(int)
    # alpha_masks = np.concatenate([alpha_masks, inverse_mask[np.newaxis, ...]], axis=0)

    # for i, alpha_mask in enumerate(alpha_masks):
    #     if i < len(alpha_mask) - 1:
    #         alpha_mask = np.logical_not(alpha_mask).astype(int)
    #     # else:
    #     #     alpha_mask = np.ones_like(alpha_mask)
...
def color_mask(alpha_masks: np.ndarray,   original_image: Image.Image) -> Image.Image:
    # alpha maks is a b, h, w size image
    masked_images = []
    original_image = np.asarray(original_image)
    for alpha_mask in alpha_masks:
        ...
        # alpha_mask = np.logical_not(alpha_mask).astype(int)
        # image = np.where(alpha_mask[:, :, None], red_mask, original_image)
        binary_mask = np.where(alpha_mask > 0.5,red_mask,0).astype(np.uint8)
        masked_images.append(Image.fromarray(binary_mask))
    return masked_images
